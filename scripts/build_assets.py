...
def render_icon(name: str, svg: Path, w: int, h: int, trim: bool) -> tuple[list[int], bytes]:
...
def main() -> None:
    ...
    svgs = {}
    icon_names = {k: v["icon"] for k, v in spec["icons"].items()}
    for icon in sorted(set(icon_names.values())):
        svgs[icon] = fetch(f"{spec['sources']['meteocons']['base']}/{icon}.svg",
    ...
    for name, ic in spec["icons"].items():
        ...
        icons[name] = (w, h) + render_icon(name, svgs[ic["icon"]], w, h, ic.get("trim", False))

...
- **Settings Schema** — settings the App registers in code (key, type, label, default, range); registration promises API + persistence + stock UI: the served Settings UI auto-renders them in an "App" section, persistence flows through the web server into the Config Store, and the App is notified on change. A setting registered **hidden** keeps the API and persistence but opts out of stock rendering — for values a custom skin owns or machine-facing state. Raw Config Store access remains underneath for what the schema can't express.

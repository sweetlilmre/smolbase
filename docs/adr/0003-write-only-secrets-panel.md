...
- Descriptors are firmware-side strings in flash; renaming a label is free,
  renaming a *key* orphans the stored NVS value (it stays until factory
  reset or a curl clear) — the descriptor key is effectively an API.

...
**Written:** 2026-08-24, after running the experiment described below on hardware. **Status: root cause identified, proven by patch-and-measure, and fixed.**
...
**Harness:** `spike/mbedtls-perf/` · **Captures:** `spike/mbedtls-perf/results/*.log` · **Patch as measured:** `spike/mbedtls-perf/upstream/as-measured.patch`

## The finding in one paragraph

mbedTLS 4.1.0 is 15–41% slower than 3.6.6 per bignum primitive on ESP32, at **identical call counts**. The entire gap comes from **three lines** changed in 2024 by two commits that made `mbedtls_mpi_core_sub()` and `mbedtls_mpi_core_mla()` constant-time. Those lines call `mbedtls_ct_uint_lt()`, which has hand-written assembly for Arm, AArch64, x86-64 and x86 — and **for every other architecture falls back to a generic C implementation built on six `asm volatile` optimisation barriers**. On Xtensa that generic version is additionally too complex for GCC to inline at `-Os`, so each use becomes an out-of-line `call8` with a register-window entry and four stack spills. The result is ~44 CPU cycles where mbedTLS 3.6.6 used a single compare. Reverting the three lines recovers 100% of the gap; a fix that keeps the constant-time guarantee recovers **111%** — mbedTLS 4.1.0 then beats 3.6.6 by 3.9% on an ECDSA verify.
...
The exact diff that produced the measurements is `spike/mbedtls-perf/upstream/as-measured.patch`, against TF-PSA-Crypto as vendored in ESP-IDF v6.0.2 (base commit in `BASE-COMMIT.txt`). It uses an `#elif 1` shim to keep the original generic branch visible for comparison; an upstream patch should replace the `#else` body outright.
...
- **Constant-time behaviour of the fix is argued, not verified.** The replacement has no branch and no comparison, and `_if_else_0` is a plain AND — but I have not run mbedTLS's constant-flow test suite (Valgrind/MemSan) against it. Upstream must, before accepting it. This matters: the code being changed exists specifically to be constant-time.

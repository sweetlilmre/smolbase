...
**3. The Espressif fix — [espressif/esp-idf#19027](https://github.com/espressif/esp-idf/pull/19027), opened as draft 2026-08-29.** From [`sweetlilmre/esp-idf:fix/mpi-hw-min-bit-len`](https://github.com/sweetlilmre/esp-idf/tree/fix/mpi-hw-min-bit-len), one commit (`26338f54` — retitled to conventional-commit format for ESP-IDF's linter, same tree as the verified original), one file, +100/−0. CLA signed. (#19026 was the first attempt, auto-closed by a shallow-clone amend mishap — see the note in [espressif-hardware-mpi-report-draft.md](espressif-hardware-mpi-report-draft.md).) Description ready in [espressif-hardware-mpi-report-draft.md](espressif-hardware-mpi-report-draft.md); the patch itself is also saved at `spike/mbedtls-perf/upstream/esp-idf-mpi-min-bitlen.patch`.
...
- **The 512-bit threshold is validated on a second chip (2026-08-30).** An ESP32-S3 (LX7) ran the full sweep with the accelerator on, off, and on-with-the-fix: its crossover is at 512 bits almost exactly (software 23.55 µs against hardware 23.64 at 512; software ahead 1.8× at 256 and 1.18× at 384), and the fix lands ECC within 2–5% of software speed while RSA keeps its 2.7× hardware win. Full tables and captures: [espressif-mpi-threshold-open-question.md](espressif-mpi-threshold-open-question.md) (now answered) and `results/idf6-s3-mpi-{on,off,on-fix}.log`. A 384-bit `mpi_mul` point was added to the harness sweep for this.

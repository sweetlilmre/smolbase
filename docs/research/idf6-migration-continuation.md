...
**Branch:** `idf6-migration`, ~94 commits ahead of `main` — this note is near the tip. **The spike instrumentation is gone**: the 2026-08-30 cleanup commit removed the `src/core/` probes, de-instrumented `Http.cpp`, reverted the SDK-tree counters and the v0.3.3 worktree, and folded the last research findings into `sdkconfig.defaults` comments. The first *code* change since `3bf86f6` is that cleanup; everything between is research and documentation.

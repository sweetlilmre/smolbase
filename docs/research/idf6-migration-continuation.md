...
3. **Worth reporting upstream:** mbedTLS 4 / TF-PSA-Crypto is roughly 2× slower than mbedTLS 3 on an ECDHE-ECDSA handshake on ESP32, and the RSA/MPI accelerator makes it worse rather than better. Searched `espressif/esp-idf` and `Mbed-TLS/mbedtls` and found nothing matching — but that was a keyword search, so treat it as weak evidence and do not assert it is undiscovered.
...
- **`CONFIG_MBEDTLS_HARDWARE_MPI` is deliberately OFF, and that is a speed-up.** All four permutations, measured with the buffered reader in place:
  ...
  The peripheral is an *RSA* accelerator. Its only effect on an ECDSA handshake is intercepting `mbedtls_mpi_mul_mpi`, and the port has **no lower size threshold**: every 256-bit multiply pays a crypto-lock acquire, a clock enable, two hardware passes (the ESP32 needs two), a clock disable and a lock release. mbedTLS 4's ECP calls it far more often than 3.x did, so the fixed per-call overhead multiplies out.
- **The two levers interact.** Fixed-point ECP is worth 28% with the accelerator off but only 5% with it on, because the hardware overhead masks the precomputation. Measure them together.

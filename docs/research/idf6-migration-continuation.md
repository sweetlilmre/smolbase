...
3. **Worth reporting upstream, with the corrected number.** mbedTLS 4.1.0 / TF-PSA-Crypto is **23–34% slower than 3.6.6 per P-256 ECDSA verify** on this chip, at identical call counts — not the "roughly 2×" this note used to claim. That 2× was the contaminated end-to-end figure, ~2.07 s of which was our own unbuffered reader. The per-primitive regressions are: software `mpi_mul` at 256 bits +15%, `mpi_inv_mod` +41%, `mpi_exp_mod` at 2048 bits +21%, `ecp_mul` +22–33%. The accelerator point is separate and is not version-specific (above). Searched `espressif/esp-idf` and `Mbed-TLS/mbedtls` and found nothing matching — but that was a keyword search, so treat it as weak evidence and do not assert it is undiscovered.
